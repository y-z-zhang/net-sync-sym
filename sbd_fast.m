...
%% matrices A within the field \mathbb{R} or \mathbb{C}. The matrices do not need to
%% be symmetric or hermitian. In all cases, the transformation matrix P is
%% orthogonal/unitary and the resulting SBD is the finest one achievable with an
%% orthogonal/unitary transformation:
%%   -- For symmetric or hermitian matrices, this is also the finest SBD achievable
%%      with any invertible transformation.
%%   -- For generic matrices (not necessarily symmetric or hermitian), the code
%%      simultaneously block diagonalizes the matrices and their (conjugate)
%%      transposes, so the SBD is the finest in the sense of the matrix *-algebra
%%      generated by the set. A non-orthogonal similarity transformation can
%%      sometimes achieve finer blocks for non-normal matrices, but at the cost of
%%      P no longer being orthogonal/unitary.
%% (See Proposition 2.1 of the reference below for the symmetric/hermitian case and
%% its concluding remarks for a discussion of the non-self-adjoint case.)

%%%%%%
%USAGE
%%%%%%
%% P = sbd_fast(A,field,threshold)
%% A --- the cell array containing the set of matrices to be simultaneously block diagonalized
%% field --- 'real' or 'complex'
%% threshold --- error tolerance for linear independence test (suitable choice makes the algorithm robust against noises in matrix data)
...
function P = sbd(A,field,threshold)
  ...
  while size(P,2) < n
    ...
    %% find linearly independent vectors among v_test and its images under A as well as A', make them orthonormal
    ...
    %% attempt to expand v to form an invariant subspace under matrices in A
    flag = 1;
    while flag == 1
